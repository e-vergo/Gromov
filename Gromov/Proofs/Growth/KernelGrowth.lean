...
variable (S : Set G) [Fintype S] (φ : G →* Multiplicative ℤ) (hφ : Function.Surjective φ)

/-- Level set of φ: all elements with a given φ-value. -/
def levelSet (k : ℤ) : Set G :=
  { g : G | Multiplicative.toAdd (φ g) = k }

/-- Level sets partition G. -/
theorem levelSet_partition : ∀ g : G, ∃! k : ℤ, g ∈ levelSet φ k := by
  intro g
  exact ⟨Multiplicative.toAdd (φ g), rfl, fun k hk => hk.symm⟩
...
theorem levelSet_intersection_bound (t : G) (ht : φ t = Multiplicative.ofAdd 1) (k : ℤ) (n : ℕ) :
...
theorem slicing_sum (n : ℕ) (M : ℕ) (hM : ∀ g ∈ CayleyBall S n, (Multiplicative.toAdd (φ g)).natAbs ≤ M) :
...
theorem average_slice_bound (n : ℕ) (M : ℕ) (hM : M > 0)
    (hM_bound : ∀ g ∈ CayleyBall S n, (Multiplicative.toAdd (φ g)).natAbs ≤ M) :
    ∃ k : ℤ, (levelSetIntersection S φ k n).ncard * (2 * M + 1) ≥ (CayleyBall S n).ncard := by
  sorry

/-- Key bound: |B_K(n)| × (number of levels) ≤ C|B_G(Cn)|. -/
theorem kernel_times_levels_le_group (t : G) (ht : φ t = Multiplicative.ofAdd 1) (n : ℕ) (hn : n > 0) :
...
theorem kernel_ball_subset_parent_ball (t : G) (ht : φ t = Multiplicative.ofAdd 1) (n : ℕ) :
    let C := 1 + 2 * generatorPhiBound S φ
...
theorem kernel_ball_size_bound (t : G) (ht : φ t = Multiplicative.ofAdd 1) (n : ℕ) :
    let C := 1 + 2 * generatorPhiBound S φ
...
theorem kernel_growth_over_n (t : G) (ht : φ t = Multiplicative.ofAdd 1) (n : ℕ) (hn : n > 0) :
    let C := 1 + 2 * generatorPhiBound S φ
    ∃ C' : ℝ, C' > 0 ∧
...
theorem kernel_finite_of_linear_growth
    ...
    (MonoidHom.ker φ).carrier.Finite := by
  sorry
...
theorem descent_terminates (hpoly : HasPolynomialGrowth G) :
    ∃ (H : Subgroup G), H.FiniteIndex ∧
      (H.carrier.Finite ∨ ∃ (K : Subgroup H), K.FiniteIndex ∧ Group.IsNilpotent K) := by
  sorry
